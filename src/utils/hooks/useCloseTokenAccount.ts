...
export const useCloseTokenAccount = () => {
  ...
  const closeTokenAccount = async (tokenAccountPubkey: PublicKey): Promise<TransactionInstruction | null> => {
    ...
    try {
      ...
      console.warn("Failed to fetch token account info:", error);
  ...
  const closeTokenAccountsAndSendTransaction = async (
    ...
    try {
      ...
      try {
        ...
        console.warn("Failed to get blockhash:", error);
        ...
        for (let i = 0; i < signedTransactions.length; i++) {
          try {
            ...
            if (confirmation.value.err) {
              console.warn(`Transaction ${signature} failed to confirm:`, confirmation.value.err);
              continue;
            }
            successfulTxs++;
          } catch (error) {
            console.warn(`Error processing transaction ${i + 1}:`, error);
            continue;
          }
        }
      ...
      console.warn("Failed to close token accounts:", error);
