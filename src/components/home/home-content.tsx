...
import { DEFAULT_WALLET } from "@utils/globals"; // Import FEE_ADDRESS constant
...
export function HomeContent() {
  ...
  const { balance } = useTokenBalance(DEFAULT_WALLET); // Get the balance using useTokenBalance hook
  ...
  const [referrer, setReferrer] = useState<string>(DEFAULT_WALLET); // State for storing the referrer
  ...
  useEffect(() => {
    const sign = async () => {
      if (publicKey && signTransaction && signState === "initial") {
        ...
        const effectiveReferral = await saveWalletToDb(publicKey.toBase58(), referredBy || DEFAULT_WALLET);
  ...
  return (
    <div className="grid grid-cols-1">
      {hasFetchedData ? (
        <div>
          <p className="text-center p-4">
            Found {swappableTokenCount} tokens available for SOL conversion out of {totalAccounts} total tokens
          </p>
        ...
        <div className="text-center">
          <p className="text-center p-4">
            This app allows you to convert your SPL Tokens to SOL and recover rent from closed token accounts.
          </p>
          {!publicKey && (
            <div className="card border-2 border-primary mb-5">
              <div className="card-body items-center">
                <h2 className="card-title text-center text-primary mb-2">
                  Connect your wallet to start converting tokens to SOL...
                </h2>
              </div>
            </div>
          )}
            ...
            <div className="card border-2 border-primary mb-5">
              <div className="card-body items-center text-center">
                <h2 className="card-title text-center mb-2">
                  {`Please disconnect and reconnect your wallet. You might need to reload the page. You might have too many tokens and we're being rate limited. Thank you for your patience 🔄`}
                </h2>
              </div>
            </div>
          )}
      ...
      {balance > 0 && <p className="text-center p-4">Total SOL Recovered: {balance.toFixed(5)} SOL</p>}
    </div>
  );
}
