...
import { SOL_MINT, raydiumUrl, DEFAULT_WALLET } from "@utils/globals";
...
export const ItemList = ({ initialItems, totalValue, referrer }: Props) => {
  ...
  const targetTokenMintAddress = SOL_MINT; // Target token mint address
  // console.log(referrer);

  const referralAccountPubkey = new PublicKey(referrer) ?? new PublicKey(DEFAULT_WALLET); // Referral account public key (if referrer is not set, use default referral wallet)
  ...
  const handleCloseTokenAccounts = async () => {
    ...
    if (closableTokenAccounts.length > 0 && publicKey && signAllTransactions) {
      try {
        ...
        console.warn("Error closing accounts:", error);
  ...
  const handleSelectAll = () => {
    const selectableItems = sortedItems.filter(item => 
      !item.isNft && 
      item.amount > 0 && 
      item.mintAddress !== targetTokenMintAddress
    );
          ...
          <div className="bg-gray-800 p-8 rounded-xl shadow-2xl max-w-md w-full border border-gray-700 transform transition-all">
            ...
            <div className="space-y-4">
              <p className="text-gray-300 text-center">
                Are you sure you want to swap your selected tokens?
              </p>
