...
import { REFERAL_ADDRESS, SOL_MINT } from "@utils/globals";
...
const getReferredBy = async (user: string | string[] | undefined, referrer: string | string[] | undefined) => {
  ...
  try {
    ...
    console.warn('Error getting referral:', error);
...
const ReferralPage = () => {
    ...
    useEffect(() => {
        if (balanceError) {
            console.warn('Error loading balance:', balanceError);
    ...
    useEffect(() => {
        const detectReferralAccount = async () => {
            ...
            try {
                ...
                console.warn('Error in detectReferralAccount:', error);
                setAccountExists(false);
                setNewReferralAccount(null);
                setHasTokenAccount(false);
            } finally {
    ...
    const handleCreateReferralAccount = async () => {
        ...
        try {
            ...
            const result = await createTokenReferralAccount(wallet, walletAddress, SOL_MINT);
