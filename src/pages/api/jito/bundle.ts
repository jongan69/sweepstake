...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
            ...
            cleanup = client.onBundleResult(
                ...
                (error: Error) => {
                    if (!error.message.includes('CANCELLED')) {
                        console.warn('Bundle error in listener:', error);
        ...
        console.warn('Bundle error details:', error);
        if (error.result) {
            console.warn('Bundle result details:', JSON.stringify(error.result, null, 2));
        }
